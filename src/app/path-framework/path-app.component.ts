...
export abstract class PathAppComponent implements IPathApp {
    ...
    protected _pageStack: Page[] = [];
    protected _formStack: Form[] = [];
    protected _userId!: string;
    protected _texts: { [key: string]: any[] } = {};
    ...
    public abstract getGuiModel(): any;
    ...
    private loadApplicationTexts() {
        this._texts["Logout"] = <any>this.translationService.getText("Logout");
        this._texts["NotSignedIn"] = <any>this.translationService.getText("NotSignedIn");
        this._texts["SignedInAs"] = <any>this.translationService.getText("SignedInAs");
    }
    ...
    public login(event: any, userId: string, password: string) {
        ...
        this.pathService.serverPost(this.getBackendUrl(), "/login", credentials, (data: any) => {
            ...
            this._userId = userId;
            this.loadApplicationTexts();
            this.setCurrentPage(this.getStartPage(), <any>null); // set start page
        }, (err: any) => {
    ...
    public logout() {
        ...
        this._userId = <any>null;
        location.reload();
    }

    public showUserForm() {
        this.setCurrentForm(this.getOwnUserForm(), new Key(0, "userId"), <any>null, <any>null); // TODO set correct key
    }
    ...
    private refreshPageStack(index: number, clearSearch: any, afterRefreshHandler: () => void) {
        ...
        if (this._pageStack[index].id === this.getStartPage() && clearSearch) {
            ...
            refresh = (element: PageElement) => {
                ...
                list.filterChanged(<any>null);
                list.refresh(<any>null, afterRefreshHandler);
            };
        } else {
    ...
    public createPageElement(modelElement: any, parentPageElement: PageElement, page: Page): PageElement[] {
        ...
        switch (modelElement.type) {
            ...
            case "list":
                ...
                if (!dynamicList.searchRequired) {
                    dynamicList.refresh(<any>null, <any>null);
                }
            ...
            case "ChartElement":
                ...
                chart.url = KeyUtility.translateUrl(modelElement["url"], <any>null, false, parentPageElement);
                elements.push(chart);
                break;
            case "pageLabel":
            ...
            case "elementList":
                ...
                const elementListUrl: any = KeyUtility.translateUrl(modelElement["url"], <any>null, false, parentPageElement);
            ...
            case "buttonGroup":
                ...
                if (modelElement["buttons"]) {
                    for (const buttonItem of modelElement["buttons"]) {
                        const buttonGroups: ButtonGroup[] = <ButtonGroup[]>this.createPageElement(buttonItem, parentPageElement, <any>null);
        ...
        for (const element of elements) {
            if (modelElement["permissionUrl"] != null) {
                ...
                const permissionUrl: string = KeyUtility.translateUrl(modelElement["permissionUrl"], <any>null, false, parentPageElement);
                const permissionHandler = (permissionElement: PageElement) => (data: any) => {
                    permissionElement.visible = data["permission"];
                };
                this.pathService.serverGet(this.getBackendUrl(), permissionUrl, permissionHandler(element), <any>null);
            }
            if (!element.type) {
    ...
    public createForm(formId: string,
        ...
        if (modelForm != null) {
            ...
            if (this.getBeans()[formId] != null && this.getHandlers()[handlerName] != null) {
                const formBean: any = new (this.getBeans()[formId]);
                ...
                for (let a = 0; a < form.fields.length; a++) {
                    if (form.fields[a].id != null) {
                        var parameter: any = <any>form.fields[a].id;
                        formBean[parameter] = <any>form.fields[a];
                    }
                }
                form.bean = formBean;
    ...
    private createFormField(modelFormField: any, form: Form, parentPageElement: IPageElement): FormField {
        let formField: FormField = <any>null;
        switch (modelFormField.type) {
            ...
            case "RadioGroupField": {
                ...
                if (modelFormField["url"] != null) {
                    ...
                    this.pathService.serverGet(this.getBackendUrl(), radiosUrl, radioLoaderForField, <any>null);
                } else {
        ...
        if (modelFormField["permissionUrl"] != null) {
            ...
            this.pathService.serverGet(formField.getForm().getApp().getBackendUrl(), permissionUrl, permissionHandler(formField), <any>null);
        }
